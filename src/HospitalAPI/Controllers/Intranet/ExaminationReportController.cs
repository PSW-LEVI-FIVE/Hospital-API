...
namespace HospitalAPI.Controllers.Intranet
{
    ...
    public class ExaminationReportController: ControllerBase
    {
        ...
        public async Task<IActionResult> Create(CreateExaminationReportDTO reportDto)
        {
            ...
            ExaminationReport report = await _examinationReportService.Create(reportDto.MapInitialToModel());
            return Ok(report);
        }

        [HttpPatch]
        [Route("report")]
        public async Task<IActionResult> Update(CreateExaminationReportDTO reportDto)
        {
            reportDto.DoctorId = GetCurrentUser().Id;
            ExaminationReport report = await _examinationReportService.Update(reportDto.MapToModel());
            return Ok(report);
        }
